...
`,bt="meshtastic",wt="hermes";function xt(t,e){const n=t.entities;return n?Object.values(n).filter(t=>t.platform===e):Object.keys(t.states).filter(t=>t.includes(e)).map(t=>({entity_id:t,platform:e}))}function kt(t,e){for(const n of xt(t,wt))if(n.entity_id.endsWith(e)){const e=t.states[n.entity_id];if(e)return e}}function Pt(t,e){const n=t.devices?.[e];if(!n?.identifiers)return null;for(const t of n.identifiers)if(t?.[0]===bt){const e=Number.parseInt(String(t[1]),10);return Number.isNaN(e)?null:e}return null}function zt(t){const e=(t.split(".")[1]??t).split("_");return e.slice(Math.max(e.length-2,0)).join(" ")}function Lt(t){const e=new Map;for(const n of xt(t,bt)){const i=n.device_id;if(!i)continue;const s=t.states[n.entity_id];if(!s)continue;let o=e.get(i);if(!o){const n=t.devices?.[i];o={deviceId:i,nodeNum:Pt(t,i),name:n?.name_by_user||n?.name||s.attributes?.friendly_name||i,values:{}},e.set(i,o)}o.values[zt(n.entity_id)]=s}return[...e.values()].sort((t,e)=>t.name.localeCompare(e.name))}function Tt(t,e,n=120){if(!e)return!1;const i=t.states[e];if(!i||"unavailable"===i.state||"unknown"===i.state)return!1;const s=Date.parse(i.state);return!Number.isNaN(s)&&Date.now()-s<=60*n*1e3}const St=[/\{state:([^:}]+)\}/g,/\{attr:([^:}]+):[^}]+\}/g,/\{do:[^:}]+:([^:}]+?)(?::[^}]*)?\}/g];function $t(t){const e=t.attributes?.unit_of_measurement;return e?`${t.state} ${e}`:t.state}const Et="missing";function At(t,e){const n=function(t){const e=new Map,n=(t,n)=>{const i=t.trim();if(!i.includes("."))return;const s=e.get(i)??[];s.includes(n)||s.push(n),e.set(i,s)};for(const e of t)for(const t of e.commands??[]){const e=t.keyword||"?",i=t.target?.entity_id;"string"==typeof i&&n(i,e);const s=t.reply_template??"";for(const t of St){let i;for(t.lastIndex=0;null!==(i=t.exec(s));)n(i[1],e)}}return new Map([...e.entries()].sort((t,e)=>t[0].localeCompare(e[0])))}(t.entries),i=function(t){const e=[];for(const n of xt(t,wt)){const i=t.states[n.entity_id];i&&e.push(i)}return e.sort((t,e)=>t.entity_id.localeCompare(e.entity_id))}(t.hass),s=[...n.entries()].map(([e,n])=>{const i=t.hass.states[e];return{entityId:e,keywords:n,state:i,status:i?"unavailable"===i.state||"unknown"===i.state?i.state:"":Et}}),o=s.filter(t=>""!==t.status).length;return W`
            ...
            <div class="rows">
              ${s.map(t=>W`
                  <div class="row">
                    ...
                    <span class="v">
                      ${t.status===Et?e("ha.missing"):t.state?$t(t.state):""}
                    </span>
                  </div>
                `)}
            </div>
  ...
  `}(t,n,e))}
  `}function Qt(t,e,n){if(!function(t){return xt(t,wt).length>0}(t))return W`<div class="empty">${n("status.noIntegration")}</div>`;const i=Lt(t),s=kt(t,"commands_executed"),o=kt(t,"last_command"),a=kt(t,"last_error"),r=t=>t&&"unknown"!==t&&"unavailable"!==t?t:n("status.none");return W`
    <h2 class="screen-title">${n("status.title")}</h2>
    <div class="grid">
      <div class="panel stat">
        <div class="label">${n("status.nodes")}</div>
        <div class="value">${i.length}</div>
      </div>
      <div class="panel stat">
        <div class="label">${n("status.executed")}</div>
        <div class="value">${s?s.state:"0"}</div>
      </div>
      <div class="panel stat">
        <div class="label">${n("status.lastCommand")}</div>
        <div class="value small">${r(o?.state)}</div>
      </div>
      <div class="panel stat">
        <div class="label">${n("status.lastError")}</div>
        <div class="value small">${r(a?.state)}</div>
      </div>
    </div>

    ${e.map(t=>function(t,e){const n=t.last_seen,i=t.seen_counts??{},s=Object.values(i).reduce((t,e)=>t+e,0),o=s>0&&!i.accepted;return W`
    <div class="section" style="margin-top:18px">
      <div class="section-title">
        ${e("status.reception")}
        ${o?W`<span class="warn-badge">${e("status.mismatch")}</span>`:""}
      </div>
      <div class="panel">
        <div class="rows">
          <div class="row">
