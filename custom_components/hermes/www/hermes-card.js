...
`,bt="meshtastic",xt="hermes";function wt(t,e){const n=t.entities;return n?Object.values(n).filter(t=>t.platform===e):Object.keys(t.states).filter(t=>t.includes(e)).map(t=>({entity_id:t,platform:e}))}function Pt(t,e){for(const n of wt(t,xt))if(n.entity_id.endsWith(e)){const e=t.states[n.entity_id];if(e)return e}}function kt(t,e){const n=t.devices?.[e];if(!n?.identifiers)return null;for(const t of n.identifiers)if(t?.[0]===bt){const e=Number.parseInt(String(t[1]),10);return Number.isNaN(e)?null:e}return null}function zt(t){const e=(t.split(".")[1]??t).split("_");return e.slice(Math.max(e.length-2,0)).join(" ")}function Lt(t){const e=new Map;for(const n of wt(t,bt)){const i=n.device_id;if(!i)continue;const s=t.states[n.entity_id];if(!s)continue;let o=e.get(i);if(!o){const n=t.devices?.[i];o={deviceId:i,nodeNum:kt(t,i),name:n?.name_by_user||n?.name||s.attributes?.friendly_name||i,values:{}},e.set(i,o)}o.values[zt(n.entity_id)]=s}return[...e.values()].sort((t,e)=>t.name.localeCompare(e.name))}function Tt(t,e,n=120){if(!e)return!1;const i=t.states[e];if(!i||"unavailable"===i.state||"unknown"===i.state)return!1;const s=Date.parse(i.state);return!Number.isNaN(s)&&Date.now()-s<=60*n*1e3}const Et=[/\{state:([^:}]+)\}/g,/\{attr:([^:}]+):[^}]+\}/g,/\{do:[^:}]+:([^:}]+?)(?::[^}]*)?\}/g];function St(t){const e=t.attributes?.unit_of_measurement;return e?`${t.state} ${e}`:t.state}const At="missing";function $t(t,e){const n=function(t){const e=new Map,n=(t,n)=>{const i=t.trim();if(!i.includes("."))return;const s=e.get(i)??[];s.includes(n)||s.push(n),e.set(i,s)};for(const e of t)for(const t of e.commands??[]){const e=t.keyword||"?",i=t.target?.entity_id;"string"==typeof i&&n(i,e);const s=t.reply_template??"";for(const t of Et){let i;for(t.lastIndex=0;null!==(i=t.exec(s));)n(i[1],e)}}return new Map([...e.entries()].sort((t,e)=>t[0].localeCompare(e[0])))}(t.entries),i=function(t){const e=[];for(const n of wt(t,xt)){const i=t.states[n.entity_id];i&&e.push(i)}return e.sort((t,e)=>t.entity_id.localeCompare(e.entity_id))}(t.hass),s=[...n.entries()].map(([e,n])=>{const i=t.hass.states[e];return{entityId:e,keywords:n,state:i,status:i?"unavailable"===i.state||"unknown"===i.state?i.state:"":At}}),o=s.filter(t=>""!==t.status).length;return W`
            ...
            <div class="rows">
              ${s.map(t=>W`
                  <div class="row">
                    ...
                    <span class="v">
                      ${t.status===At?e("ha.missing"):t.state?St(t.state):""}
                    </span>
                  </div>
                `)}
            </div>
  ...
  `}function Mt(t,e){const n=t.filter?t.entries.filter(e=>e.direction===t.filter):t.entries;return W`
  ...
  `,t([mt({attribute:!1})],Dt.prototype,"hass",void 0),t([mt()],Dt.prototype,"value",void 0),t([mt()],Dt.prototype,"placeholder",void 0),t([mt({attribute:!1})],Dt.prototype,"domains",void 0),t([_t()],Dt.prototype,"_query",void 0),t([_t()],Dt.prototype,"_open",void 0),t([_t()],Dt.prototype,"_active",void 0),Dt=t([dt("hermes-entity-picker")],Dt);const Rt=[{id:"turn_on",label:"Turn on",service:"homeassistant.turn_on"},{id:"turn_off",label:"Turn off",service:"homeassistant.turn_off"},{id:"toggle",label:"Toggle",service:"homeassistant.toggle"}];let jt={light:[{id:"turn_on",label:"Turn on",service:"light.turn_on"},{id:"turn_on_brightness",label:"Turn on at brightness",service:"light.turn_on",value:{key:"brightness_pct",type:"number",unit:"%",min:1,max:100,step:1,default:80}},{id:"turn_off",label:"Turn off",service:"light.turn_off"},{id:"toggle",label:"Toggle",service:"light.toggle"}],switch:Rt,climate:[{id:"set_temperature",label:"Set temperature",service:"climate.set_temperature",value:{key:"temperature",type:"number",unit:"C",min:5,max:35,step:.5,default:21}},{id:"turn_on",label:"Turn on",service:"climate.turn_on"},{id:"turn_off",label:"Turn off",service:"climate.turn_off"}],cover:[{id:"open",label:"Open",service:"cover.open_cover"},{id:"close",label:"Close",service:"cover.close_cover"},{id:"stop",label:"Stop",service:"cover.stop_cover"},{id:"set_position",label:"Set position",service:"cover.set_cover_position",value:{key:"position",type:"number",unit:"%",min:0,max:100,step:5,default:100}}]},Ft={light:"light",switch:"switch",input_boolean:"switch",climate:"climate",cover:"cover"},qt=Rt;const Wt={temperature:{min:"min_temp",max:"max_temp",step:"target_temp_step"},percentage:{step:"percentage_step"},value:{min:"min",max:"max",step:"step"}},Ut={hvac_mode:"hvac_modes",preset_mode:"preset_modes",fan_mode:"fan_modes",swing_mode:"swing_modes",operation_mode:"operation_list",source:"source_list"};function Vt(t,e,n){const i=t.states[e]?.attributes??{},s={...n},o=Wt[n.key]??{};for(const t of["min","max","step"]){const e=o[t],n=e?i[e]:void 0;"number"==typeof n&&(s[t]=n)}const a=Ut[n.key],r=a?i[a]:void 0;return Array.isArray(r)&&r.length&&(s.options=r.map(String),void 0!==s.default&&s.options.includes(String(s.default))||(s.default=s.options[0])),"number"==typeof s.default&&("number"==typeof s.min&&s.default<s.min&&(s.default=s.min),"number"==typeof s.max&&s.default>s.max&&(s.default=s.max)),s}const Kt=new Set(["friendly_name","icon","supported_features","device_class","entity_picture","attribution","supported_color_modes","hs_color","rgb_color","xy_color"]);function Gt(t,e,n){if("channel"!==e.mode)return n("messages.onDm");const i=e.channel_index??0,s=t.channels.find(t=>t.index===i);return s?`${i}: ${s.name}`:`${n("settings.channel")} ${i}`}function Yt(t,e){if(t.loadError)return W`
      <div class="empty">
    ...
    ${t.editing?function(t,e,n){const i=e=>n=>t.onDraftInput(e,n.target.value);return W`
    <div class="panel">
      <div class="field">
        <label>${n("messages.keyword")}</label>
        <input .value=${e.keyword??""} @input=${i("keyword")} />
    ...
    <div class="palette">
      ...
      ${n&&t.hass.states[n]?W`
            ...
            <div class="chips">
              ...
              ${function(t,e){const n=t.states[e];return n?.attributes?Object.entries(n.attributes).filter(([t,e])=>!Kt.has(t)&&("string"==typeof e||"number"==typeof e||"boolean"==typeof e)).map(([t])=>t).sort():[]}(t.hass,n).map(e=>W`
                  <button
                    class="chip read"
            ...
            <div class="chips">
              ${function(t){const e=Ft[function(t){return t.split(".")[0]??""}(t)];return e&&jt[e]||qt}(n).map(e=>W`
                  <span class="chip-group">
                    <button
                      class="chip do"
                      @click=${()=>t.onInsert(function(t,e,n){return t.value&&void 0!==n&&""!==n?`{do:${t.service}:${e}:${t.value.key}=${n}}`:`{do:${t.service}:${e}}`}(e,n,t.paletteValues[e.id]??(e.value?Vt(t.hass,n,e.value).default:void 0)))}
                    >
                      ${e.label}
                    </button>
                    ${function(t,e,n){if(!e.value)return"";const i=Vt(t.hass,n,e.value),s=t.paletteValues[e.id]??i.default??"";if("enum"===i.type)return W`
      <select
        class="inline"
  ...
  `}(t,t.editing,e):W`
          ${n.commands.length?n.commands.map(i=>function(t,e,n,i){const s=e.service||e.reply_template||"",o="sender_dm"===e.reply_to?i("messages.onDm"):Gt(t,n,i);return W`
    <div class="list-row">
      <div class="meta">
      ...
      ${t.presets.length?t.presets.map(i=>W`
              <div class="list-row">
                <div class="meta">
                  ...
                  <span class="sub">
                    ${i.node_id?`${n("presets.toNode")} ${i.node_id}`:n("presets.toChannel")}
                  </span>
                </div>
                <div class="actions" style="margin:0">
                  <button
  ...
  `}(t,n,e)}
        `}
  `}function Jt(t,e,n,i,s=null){if(s)return W`<div class="sub-error">${s}</div>`;if(!t.length)return W`<div class="hint">${i}</div>`;const o=new Set((e??[]).map(Number));return W`
    <div class="checklist">
      ${t.map(t=>W`
          <label class="check">
            <input
  ...
  `}function Xt(t,e){const n=t.settings,i=e=>t.draftGlobal[e]??n?.[e];return W`
    <h2 class="screen-title">
      ${e("settings.title")}
      <button
        class="btn refresh"
    ...
    <div class="section">
      ...
      <div class="panel">
        ...
        <div class="field">
          ...
          ${Jt(t.nodes,i("map_nodes")??[],e=>t.onGlobalInput("map_nodes",e),e("settings.noNodes"),t.nodesError)}
    ...
    <div class="section">
      ...
      <div class="panel">
        <div class="rows">
          <div class="row">
            <span class="k">${n("settings.gateway")}</span>
            <span class="v">${e.gateway_node_id??"-"}</span>
          </div>
          <div class="row">
            <span class="k">${n("settings.mode")}</span>
            <span class="v">${e.mode}</span>
          </div>
        </div>

        ${"channel"===e.mode?function(t,e,n){const i=e.channel_index??0,s=t.channels.find(t=>t.index===i);return W`
    <div class="field" style="margin-top:12px">
      <label>${n("settings.channel")}</label>
      ${t.channels.length?W`
            <select
              @change=${n=>t.onChannelChange(e.entry_id,Number(n.target.value))}
            >
              ${t.channels.map(t=>W`
                  <option value=${t.index} ?selected=${t.index===i}>
                    ${t.index}: ${t.name}
                  </option>
                `)}
            </select>
      ...
      ${s?.default_psk?W`<div class="note warn">${n("settings.defaultPskWarning")}</div>`:""}
    </div>
  `}(t,e,n):""}
        ...
        <div class="field">
          ...
          ${Jt(t.nodes,s("authorized_nodes",e.authorized_nodes)??[],n=>t.onEntryInput(e.entry_id,"authorized_nodes",n),n("settings.noNodes"),t.nodesError)}
  ...
  `}(t,n,e))}
  `}function Qt(t,e){if(!function(t){return wt(t,xt).length>0}(t))return W`<div class="empty">${e("status.noIntegration")}</div>`;const n=Lt(t),i=Pt(t,"commands_executed"),s=Pt(t,"last_command"),o=Pt(t,"last_error"),a=t=>t&&"unknown"!==t&&"unavailable"!==t?t:e("status.none");return W`
