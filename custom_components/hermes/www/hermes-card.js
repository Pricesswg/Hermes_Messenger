...
`;function bt(t,e){const n=t.chats[e];return n?.length?n[n.length-1].ts:""}function wt(t,e,n){const i=Number(t.split(":")[1]),s=e.channels.find(t=>t.index===i);return s?`${i}: ${s.name}`:`${n("settings.channel")} ${i}`}function xt(t,e){const n=t.split(":")[1],i=e.nodes.find(t=>String(t.node_num)===n);return i?i.name:n}function kt(t,e,n,i){const s=t.chats[e]?.length??0;return W`
  ...
  `}const Pt="meshtastic",$t="hermes";function St(t,e){const n=t.entities;return n?Object.values(n).filter(t=>t.platform===e):Object.keys(t.states).filter(t=>t.includes(e)).map(t=>({entity_id:t,platform:e}))}function Lt(t,e){for(const n of St(t,$t))if(n.entity_id.endsWith(e)){const e=t.states[n.entity_id];if(e)return e}}function Tt(t,e){const n=t.devices?.[e];if(!n?.identifiers)return null;for(const t of n.identifiers)if(t?.[0]===Pt){const e=Number.parseInt(String(t[1]),10);return Number.isNaN(e)?null:e}return null}function Et(t){const e=(t.split(".")[1]??t).split("_");return e.slice(Math.max(e.length-2,0)).join(" ")}function At(t){const e=new Map;for(const n of St(t,Pt)){const i=n.device_id;if(!i)continue;const s=t.states[n.entity_id];if(!s)continue;let o=e.get(i);if(!o){const n=t.devices?.[i];o={deviceId:i,nodeNum:Tt(t,i),name:n?.name_by_user||n?.name||s.attributes?.friendly_name||i,values:{}},e.set(i,o)}o.values[Et(n.entity_id)]=s}return[...e.values()].sort((t,e)=>t.name.localeCompare(e.name))}function Ct(t,e,n=120){if(!e)return!1;const i=t.states[e];if(!i||"unavailable"===i.state||"unknown"===i.state)return!1;const s=Date.parse(i.state);return!Number.isNaN(s)&&Date.now()-s<=60*n*1e3}const Mt=[/\{state:([^:}]+)\}/g,/\{attr:([^:}]+):[^}]+\}/g,/\{do:[^:}]+:([^:}]+?)(?::[^}]*)?\}/g];const Ht=["battery","voltage","snr","hops","last_heard","utilization","uptime"];function Nt(t){const e=t.attributes?.unit_of_measurement;return e?`${t.state} ${e}`:t.state}function It(t,e){const n=At(t);return n.length?W`
  ...
  `}(t,n,e))}
  `}const ae="0.24.1";function re(t,e,n,i,s,o){if(!function(t){return St(t,$t).length>0}(t))return W`<div class="empty">${o("status.noIntegration")}</div>`;const a=At(t),r=Lt(t,"commands_executed"),l=Lt(t,"last_command"),h=Lt(t,"last_error"),c=t=>t&&"unknown"!==t&&"unavailable"!==t?t:o("status.none");return W`
    <h2 class="screen-title">
      ${o("status.title")}
