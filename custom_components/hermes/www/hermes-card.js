...
`,bt="meshtastic",wt="hermes";function xt(t,e){const n=t.entities;return n?Object.values(n).filter(t=>t.platform===e):Object.keys(t.states).filter(t=>t.includes(e)).map(t=>({entity_id:t,platform:e}))}function kt(t,e){for(const n of xt(t,wt))if(n.entity_id.endsWith(e)){const e=t.states[n.entity_id];if(e)return e}}function zt(t,e){const n=t.devices?.[e];if(!n?.identifiers)return null;for(const t of n.identifiers)if(t?.[0]===bt){const e=Number.parseInt(String(t[1]),10);return Number.isNaN(e)?null:e}return null}function Pt(t){const e=(t.split(".")[1]??t).split("_");return e.slice(Math.max(e.length-2,0)).join(" ")}function Lt(t){const e=new Map;for(const n of xt(t,bt)){const i=n.device_id;if(!i)continue;const s=t.states[n.entity_id];if(!s)continue;let o=e.get(i);if(!o){const n=t.devices?.[i];o={deviceId:i,nodeNum:zt(t,i),name:n?.name_by_user||n?.name||s.attributes?.friendly_name||i,values:{}},e.set(i,o)}o.values[Pt(n.entity_id)]=s}return[...e.values()].sort((t,e)=>t.name.localeCompare(e.name))}function $t(t,e,n=120){if(!e)return!1;const i=t.states[e];if(!i||"unavailable"===i.state||"unknown"===i.state)return!1;const s=Date.parse(i.state);return!Number.isNaN(s)&&Date.now()-s<=60*n*1e3}const St=[/\{state:([^:}]+)\}/g,/\{attr:([^:}]+):[^}]+\}/g,/\{do:[^:}]+:([^:}]+?)(?::[^}]*)?\}/g];function Tt(t){const e=t.attributes?.unit_of_measurement;return e?`${t.state} ${e}`:t.state}const At="missing";function Et(t,e){const n=function(t){const e=new Map,n=(t,n)=>{const i=t.trim();if(!i.includes("."))return;const s=e.get(i)??[];s.includes(n)||s.push(n),e.set(i,s)};for(const e of t)for(const t of e.commands??[]){const e=t.keyword||"?",i=t.target?.entity_id;"string"==typeof i&&n(i,e);const s=t.reply_template??"";for(const t of St){let i;for(t.lastIndex=0;null!==(i=t.exec(s));)n(i[1],e)}}return new Map([...e.entries()].sort((t,e)=>t[0].localeCompare(e[0])))}(t.entries),i=function(t){const e=[];for(const n of xt(t,wt)){const i=t.states[n.entity_id];i&&e.push(i)}return e.sort((t,e)=>t.entity_id.localeCompare(e.entity_id))}(t.hass),s=[...n.entries()].map(([e,n])=>{const i=t.hass.states[e];return{entityId:e,keywords:n,state:i,status:i?"unavailable"===i.state||"unknown"===i.state?i.state:"":At}}),o=s.filter(t=>""!==t.status).length;return W`
  ...
  `}(t,n,e))}
  `}const Qt="0.22.0";function te(t,e,n,i,s){if(!function(t){return xt(t,wt).length>0}(t))return W`<div class="empty">${s("status.noIntegration")}</div>`;const o=Lt(t),a=kt(t,"commands_executed"),r=kt(t,"last_command"),l=kt(t,"last_error"),h=t=>t&&"unknown"!==t&&"unavailable"!==t?t:s("status.none");return W`
    <h2 class="screen-title">
      ${s("status.title")}
