...
`;function be(e,t){const n=e.chats[t];return n?.length?n[n.length-1].ts:""}function we(e,t,n){const i=Number(e.split(":")[1]),s=t.channels.find(e=>e.index===i);return s?`${i}: ${s.name}`:`${n("settings.channel")} ${i}`}function xe(e,t){const n=e.split(":")[1],i=t.nodes.find(e=>String(e.node_num)===n);return i?i.name:n}function ke(e,t,n,i){const s=e.chats[t]?.length??0;return F`
            ...
            <div class="rows">
              ${s.map(e=>F`
                  <div class="row">
                    ...
                    <span class="v">
                      ${e.status===Re?t("ha.missing"):e.state?Ie(e.state):""}
                    </span>
                  </div>
                `)}
            </div>
  ...
  `}(e,n)}
      ...
      <div class="field">
        ...
        ${function(e,t){const n=e.entries.find(t=>t.entry_id===e.selectedEntry)??e.entries[0],i=[];for(const e of n?.commands??[]){const t=(e.group??"").trim();t&&!i.includes(t)&&i.push(t)}if(!i.length)return F``;const s=(t.group??"").trim();return F`
    <div class="chips" style="margin-bottom:2px">
      ${i.map(t=>F`
          <button
            class="chip"
            data-on=${t===s?"1":"0"}
            @click=${()=>e.onDraftInput("group",t===s?"":t)}
          >
            ${t}
          </button>
        `)}
  ...
  `}(e,n,t))}
  `}const mt="1.3.1";function gt(e,t,n,i,s){if(!Te(e))return F`<div class="empty">${s("status.noIntegration")}</div>`;const a=Ee(e),o=Le(e,"commands_executed"),r=Le(e,"last_command"),l=Le(e,"last_error"),c=e=>e&&"unknown"!==e&&"unavailable"!==e?e:s("status.none");return F`
    <h2 class="screen-title">
      ${s("status.title")}
